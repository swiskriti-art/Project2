...
Creating factors for categorical vars
...
```
ggplot for profit per month for category with facet
...
```
ggplot for box plot for loss per category
...
```
 ggplot for boxplot for Profit per category
...
```
ggplot for profit per sub-category
...
ggplot(data = store_data, 
  ...
  theme(axis.text.x = element_text(angle = 45, vjust = 0.6)) 
```
ggplot for trend for profit and selling price
